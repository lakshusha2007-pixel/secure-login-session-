...
// Content-Security-Policy — whitelists exactly what the browser may load:
//   default-src 'self'   : only same-origin resources, no external CDNs.
//   script-src 'self'    : only our own JS file (assets/js/main.js).
//   style-src 'self'     : only our own CSS file (assets/css/style.css).
//   form-action 'self'   : forms may only submit to our own domain.
//   base-uri 'self'      : no <base> tag hijacking.
header("Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self'; form-action 'self'; base-uri 'self'; frame-ancestors 'self'");
// -----------------------------------------------------------------------------
...
<head>
    ...
    <!-- Only our own stylesheet (no frameworks, per project rules). -->
