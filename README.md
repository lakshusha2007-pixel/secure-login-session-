...
Tracked **per identity in session & MySQL database**:
- Maximum **3** failed attempts.
- Lock / Inactive for **20 minutes** (1,200 seconds).
- Counter resets on successful login and automatically after the 20-minute window expires.
- On lock, the user sees: `Too many login attempts. Account is inactive for 20 minutes.`
...
| 8 | **Multiple failed attempts** | Fail login 3 times for the same user | 4th attempt blocked with `Too many login attempts. Account is inactive for 20 minutes.` |
| 9 | **Lock expiry** | Wait 20 minutes (1,200s) after lockout, try again | Lock automatically cleared; account becomes active again |
...
- [x] Brute-force protection (3 attempts / 20-minute automatic lockout & reactivation)
...
#   s e c u r e - l o g i n - s e s s i o n -  
 